...
template <typename Pred> void bm_par_copy_if_sync(benchmark::State &state) {
  auto [src, dst] = setup_fixture(100'000'000);
  auto pred = Pred{};
  for (auto _ : state) {
    auto new_end =
...
BENCHMARK_TEMPLATE(bm_par_copy_if_split, decltype(is_odd))
    ->Apply(CustomArguments);
BENCHMARK_TEMPLATE(bm_par_copy_if_sync, decltype(is_odd))
    ->Apply(CustomArguments);

BENCHMARK_TEMPLATE(bm_std_copy_if, decltype(is_prime))->Apply(CustomArguments);
BENCHMARK_TEMPLATE(bm_par_copy_if_split, decltype(is_prime))
    ->Apply(CustomArguments);
BENCHMARK_TEMPLATE(bm_par_copy_if_sync, decltype(is_prime))
    ->Apply(CustomArguments);
