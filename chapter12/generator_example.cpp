...
template <typename T> Generator<T> take_until(Generator<T> &gen, T value) {
  for (auto &&v : gen) {
    if (v == value)
      co_return;
    co_yield v;
...
template <typename T> Generator<T> add(Generator<T> &gen, T adder) {
  for (auto &&v : gen) {
    co_yield v + adder;
  }
}
...
TEST(GeneratorExample, CombineWithRangeAlgorithms) {
  ...
  auto is_subrange = std::ranges::includes(a, v);
